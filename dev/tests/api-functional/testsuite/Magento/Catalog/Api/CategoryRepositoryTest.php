...
class CategoryRepositoryTest extends WebapiAbstract
{
    ...
    /**
     * Test for create url rewrite for all stores in create category process
     *
     * @magentoApiDataFixture Magento/Catalog/Model/Category/_files/service_category_create.php
     * @magentoApiDataFixture Magento/Store/_files/second_store.php
     */
    public function testCreateUrlRewrites()
    {
        $categoryData = [
        ...
        $result = $this->createCategory($categoryData);
        $this->assertGreaterThan(0, $result['id']);
        foreach (['name', 'parent_id'] as $fieldName) {
            $this->assertEquals(
                $categoryData[$fieldName],
                $result[$fieldName],
                sprintf('"%s" field value is invalid', $fieldName)
        ...
        // Assert that a url rewrite is auto-generated for every store for the category created from the data fixture
        foreach ([1, 2] as $storeId) {
            ...
            $this->assertNotNull($urlRewrite, sprintf('UrlRewrite missing for store with ID %d', $storeId));
            ...
            $this->assertEquals('test-category-name.html', $urlRewrite->getRequestPath());
        }
